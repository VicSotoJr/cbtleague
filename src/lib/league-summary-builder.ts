import type { AggregatedBaseStats, AggregatedPlayerMetrics, LeagueData, Player, Team } from "@/types/league";
...
function sumPlayerStats(player: Player) {
...
function aggregatePlayerStats(player: Player): AggregatedPlayerMetrics {
...
export function buildLeagueSummary(data: LeagueData) {
  ...
  for (const [seasonId, season] of Object.entries(data.seasons ?? {})) {
    const getTeamColor = (team: Team & { color?: string | null }) =>
      "color" in team && typeof (team as { color?: unknown }).color === "string"
        ? ((team as { color?: string }).color ?? null)
        : null;
    ...
    const teams = (season.teams ?? []).map((team) => ({
      ...
      color: getTeamColor(team as Team & { color?: string | null }),
